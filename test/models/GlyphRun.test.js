...
describe('GlyphRun', () => {
  ...
  test('should exact slice range return same run', () => {
    const { glyphRun } = createLatinRun({ start: 0, end: 11 });
  ...
  test('should slice containing range when start not zero', () => {
    const { glyphRun } = createLatinRun({ start: 5, end: 11 });
    const sliced = glyphRun.slice(2, 5);
  ...
  test('should correctly slice glyphs (non latin)', () => {
    const { glyphRun } = createKhmerRun({ value: 'ខ្ញុំអាចញ៉ាំកញ្ចក់បាន', start: 0, end: 21 });
    const { glyphs } = glyphRun.slice(1, 8);

    expect(glyphs.map(g => g.id)).toEqual([153, 177, 112, 248, 188, 49]);
  });
  ...
  test('should correctly slice positions (non latin)', () => {
    ...
    expect(positions).toEqual([0, 0, 0, 9.08, 4.54, 9.08]);
  });
  ...
  test('should correctly slice string indices (non latin)', () => {
    ...
    expect(stringIndices).toEqual([0, 2, 3, 4, 5, 6]);
  });
